...
void main() {
  ...
  group('Allocation preview (PR3, R10-R13)', () {
    ...
    const snapshot = AllocationPreviewData(
      planCapByType: {
        'expense': 500000,
        'saving': 300000,
        'investment': 200000,
    ...
    const zeroCapSnapshot = AllocationPreviewData(
      planCapByType: {
        'expense': 500000,
        'saving': 0,
        'investment': 200000,
    ...
    testWidgets('P1 — preview shows the projected percentage against the cap (create, healthy)', (tester) async {
      ...
      final text = tester.widget<Text>(find.textContaining('50% del plan de Gasto'));
      expect(text.style?.color, AppColors.textSecondary);
    });

    testWidgets('P2 — preview shifts to alert tone when the projected total exceeds the cap', (tester) async {
      await _pumpDialog(tester, allocationPreview: snapshot);
      ...
      final text = tester.widget<Text>(find.textContaining('110% del plan de Gasto'));
      expect(text.style?.color, AppColors.expense);
    });

    testWidgets('P3 — zero cap with a positive projected allocation warns without a raw percentage or "Infinity"', (tester) async {
      ...
      final text = tester.widget<Text>(find.textContaining('Sin plan asignado'));
      expect(text.data, contains('Ahorro'));
      expect(text.style?.color, AppColors.expense);
    });

    testWidgets('P4 — zero cap with no projected allocation shows no warning and no percentage', (tester) async {
      await _pumpDialog(tester, allocationPreview: zeroCapSnapshot);
    ...
    testWidgets('P5 — goal meta with months < 1 hides the preview (mirrors the Cuota mensual gate)', (tester) async {
      await _pumpDialog(tester, allocationPreview: snapshot);
    ...
    testWidgets('P6 — CREATE mode: months, amount, toggle, and category each independently recompute the preview', (tester) async {
      await _pumpDialog(tester, allocationPreview: snapshot);
      await _enterByLabel(tester, 'Nombre', 'X');
    ...
    testWidgets('P7 — EDIT mode: amount/months changes recompute away from the stored quota (R11.3), self-exclusion holds', (tester) async {
      final existing = Budget(
        id: 'edit1',
        userId: 'u1',
        category: 'Renta',
      ...
      await _pumpDialog(tester, existing: existing, allocationPreview: snapshot);
    ...
    testWidgets('P8 — category switch mid-create rebases to the new category cap and base sum (R10.4/R11.2)', (tester) async {
      await _pumpDialog(tester, allocationPreview: snapshot);
      ...
      final text = tester.widget<Text>(find.textContaining('150% del plan de Inversión'));
      expect(text.style?.color, AppColors.expense);
      expect(find.textContaining('80% del plan de Gasto'), findsNothing);
    });

    testWidgets('P9 — category switch mid-edit rebases to the new category, self-exclusion still holds (R11.2)', (tester) async {
      final existing = Budget(
        id: 'edit2',
        userId: 'u1',
    ...
    testWidgets('P10 — save is never blocked even while the preview is in warning state (R13.1)', (tester) async {
    ...
    testWidgets('P11 — no snapshot provided degrades to no preview (existing call sites unaffected)', (tester) async {
      await _pumpDialog(tester);
